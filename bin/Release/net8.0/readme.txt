Примечание: Полный список заказов находится в файле Orders.txt. В него можно добавлять или удалять записи. В файле config.txt находятся входные параметры, на основе которых будет происходить фильтрация. Их также можно изменять для различных результатов.
